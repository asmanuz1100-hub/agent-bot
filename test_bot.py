...
if __name__=='__main__':unittest.main()
   ...
   server_source=inspect.getsource(bot.serve_webhook)
   self.assertIn("reports.client_card_html(local,actor,cid,photo_url=photo_url)",server_source)
   self.assertIn("photo_data=customer_photo_bytes(client['photo'])",server_source)

 def test_customer_photo_proxy_checks_file_and_never_exposes_token_to_html(self):
  import io
  jpeg=bytes([255,216,255])+b'test-image'
  with patch.object(bot,'TOKEN','not-a-real-bot-token'),\
       patch.object(bot,'api',return_value={'file_path':'photos/file_12.jpg','file_size':len(jpeg)}) as api,\
       patch.object(bot.urllib.request,'urlopen',return_value=io.BytesIO(jpeg)) as urlopen:
   self.assertEqual(bot.customer_photo_bytes('telegram-file-id'),jpeg)
   api.assert_called_once_with('getFile',file_id='telegram-file-id')
   self.assertIn('/file/botnot-a-real-bot-token/photos/file_12.jpg',urlopen.call_args.args[0])
  with patch.object(bot,'api',return_value={'file_path':'../secret.txt','file_size':3}):
   with self.assertRaises(ValueError):bot.customer_photo_bytes('bad-file')
  with patch.object(bot,'api',return_value={'file_path':'photos/file_1.jpg','file_size':8_000_001}):
   with self.assertRaises(ValueError):bot.customer_photo_bytes('large-file')
